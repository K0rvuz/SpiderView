...
.card{position:absolute;border-radius:12px;background:#20252c;border:1.5px solid #404955;box-shadow:0 5px 15px #0006;overflow:hidden;cursor:pointer}.card.selected{border:2.5px solid #60a5fa}.card.hidden{display:none}.card.note{background:#24231f;border-color:#d9a441}.note-strip{position:absolute;left:0;top:0;bottom:0;width:7px;background:#d9a441}
...
<body>
<header><div><div class="brand" id="title"></div><div class="muted" id="stats"></div></div><div class="spacer"></div><input id="search" placeholder="Buscar URL, título, endpoint ou domínio"><button data-kind="page" class="kind active">Page</button><button data-kind="api" class="kind active">API</button><button data-kind="dom_state" class="kind active">DOM</button><button data-kind="note" class="kind active">Note</button><button id="fit">Fit</button></header>
<div id="app"><div id="viewport"><div id="world"><svg id="edges"></svg><div id="cards"></div></div><div id="legend">Scroll: zoom · Arraste o fundo: pan · Clique: detalhes</div></div><aside id="details"><div id="empty">Selecione um card para ver os detalhes.</div></aside></div>
<script>
...
const state={scale:1,tx:0,ty:0,panning:false,lastX:0,lastY:0,selected:null,kinds:new Set(['page','api','dom_state','note']),query:''};const nodeMap=new Map(DATA.nodes.map(n=>[n.id,n]));let minX=Infinity,minY=Infinity,maxX=0,maxY=0;
for(const n of DATA.nodes){minX=Math.min(minX,+n.x||0);minY=Math.min(minY,+n.y||0)}if(!isFinite(minX)){minX=0;minY=0}for(const n of DATA.nodes){n._x=(+n.x||0)-minX+140;n._y=(+n.y||0)-minY+140;maxX=Math.max(maxX,n._x+(n.width||320));maxY=Math.max(maxY,n._y+(n.height||250))}const WORLD_W=Math.max(1200,maxX+180),WORLD_H=Math.max(800,maxY+180);world.style.width=WORLD_W+'px';world.style.height=WORLD_H+'px';svg.setAttribute('width',WORLD_W);svg.setAttribute('height',WORLD_H);svg.innerHTML='<defs><marker id="arrow" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto-start-reverse"><path d="M 0 0 L 10 5 L 0 10 z" fill="#56616f"/></marker><marker id="noteArrow" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto-start-reverse"><path d="M 0 0 L 10 5 L 0 10 z" fill="#D9A441"/></marker></defs>';
