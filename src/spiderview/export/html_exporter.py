...
.preview{margin:10px 12px;height:132px;border-radius:8px;background:#15191e;display:flex;align-items:center;justify-content:center;overflow:hidden}.preview img{max-width:100%;max-height:100%;object-fit:contain;pointer-events:none;-webkit-user-drag:none}.card-foot{position:absolute;left:12px;right:12px;bottom:0;height:38px;border-top:1px solid #343a43;padding-top:9px;color:#9ca3af;font-size:11px;display:flex;justify-content:space-between}.note-body{padding:14px 20px 38px;color:#d6d0c2;font-size:13px;line-height:1.4;white-space:pre-wrap;overflow:hidden;height:132px}.note-tags{position:absolute;left:20px;right:12px;bottom:12px;color:#afa58f;font-size:11px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
...
function cardAnchor(n,towardX,towardY){const w=n.width||320,h=n.height||250,cx=n._x+w/2,cy=n._y+h/2,dx=towardX-cx,dy=towardY-cy;if(Math.abs(dx)<.001&&Math.abs(dy)<.001)return{x:cx,y:cy};const hw=w/2,hh=h/2,scale=1/Math.max(Math.abs(dx)/hw,Math.abs(dy)/hh);return{x:cx+dx*scale,y:cy+dy*scale}}function renderEdges(){[...svg.querySelectorAll('.edge,.edge-label')].forEach(x=>x.remove());for(const e of DATA.transitions){const s=nodeMap.get(e.source_id),t=nodeMap.get(e.target_id);if(!s||!t||!visible(s)||!visible(t))continue;const scx=s._x+(s.width||320)/2,scy=s._y+(s.height||250)/2,tcx=t._x+(t.width||320)/2,tcy=t._y+(t.height||250)/2,start=cardAnchor(s,tcx,tcy),end=cardAnchor(t,scx,scy),sx=start.x,sy=start.y,tx=end.x,ty=end.y,vx=tx-sx,vy=ty-sy,distance=Math.max(1,Math.hypot(vx,vy)),ux=vx/distance,uy=vy/distance,control=Math.max(60,Math.min(distance*.35,220)),c1x=sx+ux*control,c1y=sy+uy*control,c2x=tx-ux*control,c2y=ty-uy*control;const p=document.createElementNS('http://www.w3.org/2000/svg','path');p.setAttribute('class','edge');const isNote=Boolean(e.metadata&&e.metadata.manual_note);const noteColor=isNote?((e.metadata&&e.metadata.note_color)||(s.metadata&&s.metadata.color)||'#D9A441'):null;p.setAttribute('marker-end',isNote?'url(#'+noteMarker(noteColor)+')':'url(#arrow)');if(isNote){p.setAttribute('stroke',noteColor);p.setAttribute('stroke-dasharray','8 6')}p.setAttribute('d',`M ${sx} ${sy} C ${c1x} ${c1y}, ${c2x} ${c2y}, ${tx} ${ty}`);svg.appendChild(p);if(e.label){const label=document.createElementNS('http://www.w3.org/2000/svg','text');label.setAttribute('class','edge-label');label.setAttribute('x',(sx+tx)/2);label.setAttribute('y',(sy+ty)/2-6);label.setAttribute('text-anchor','middle');label.textContent=e.label;svg.appendChild(label)}}}
...
function transform(){world.style.transform=`translate(${state.tx}px,${state.ty}px) scale(${state.scale})`}function fit(){const shown=DATA.nodes.filter(visible);if(!shown.length)return;let left=Infinity,top=Infinity,right=-Infinity,bottom=-Infinity;for(const n of shown){left=Math.min(left,n._x);top=Math.min(top,n._y);right=Math.max(right,n._x+(n.width||320));bottom=Math.max(bottom,n._y+(n.height||250))}const pad=70,vw=viewport.clientWidth,vh=viewport.clientHeight,width=Math.max(1,right-left),height=Math.max(1,bottom-top),scale=Math.min((vw-pad*2)/width,(vh-pad*2)/height,1);state.scale=Math.max(.05,scale);state.tx=(vw-width*state.scale)/2-left*state.scale;state.ty=(vh-height*state.scale)/2-top*state.scale;transform()}
