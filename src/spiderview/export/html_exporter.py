...
.card{position:absolute;border-radius:12px;background:#20252c;border:1.5px solid #404955;box-shadow:0 5px 15px #0006;overflow:hidden;cursor:pointer}.card.selected{border:2.5px solid #60a5fa}.card.hidden{display:none}.card.note{background:#24231f;border-color:#d9a441}.card.group{background:#20262e;border-color:#46515e}.group .card-head{background:#2a323d}.note-strip{position:absolute;left:0;top:0;bottom:0;width:7px;background:#d9a441}
...
const state={scale:1,tx:0,ty:0,panning:false,lastX:0,lastY:0,selected:null,kinds:new Set(['page','api','dom_state','note','group']),query:''};const nodeMap=new Map(DATA.nodes.map(n=>[n.id,n]));let minX=Infinity,minY=Infinity,maxX=0,maxY=0;
